...
*Re-run of the Day-5 chain test using the 2026-08-10-enriched `cv-reviewer` (intake questions, So-What/Numbers/Credibility/Buzzword bullet audits, Communication Rules). Source CV is byte-identical to the original `Testing/Input/sample_cv.md`. Unlike the first pass at this re-test — which wrongly assumed no candidate was available to ask and skipped intake entirely — `references/intake-questions.md` was actually run this time, live, with the user answering on Maria's behalf.*

### Live Intake Q&A

Per `references/intake-questions.md`'s own prioritization note ("target role and content confidence usually matter more than format history"), the highest-value batch was asked first:

| Question (intake-questions.md #) | Answer |
|---|---|
| Target role (#6) | Backend / Software Engineer |
| Actual day-to-day tech, for the vaguest entries (#11) | Python/Django + MySQL |
| Tools used but not listed (#14) | Confirmed: a web framework (Django) + basic AWS usage |
| Anything indefensible in an interview (#13) | Confirmed: the "significantly" in the API-performance bullet — no real number was ever tracked for it |

That last answer confirms, rather than just flags, what the Credibility Audit suspected below — this is the one finding in this report that changed from "possible" to "confirmed" as a direct result of actually running intake instead of assuming it away.
...
The document is plain, single-column text with no tables, graphics, or text boxes — a clean baseline. But one structural issue is severe enough to cap this score well below "Strong": the second job (PixelWorks Studio) is filed under a custom header, **"MY JOURNEY,"** instead of continuing under "Experience." Most ATS platforms parse fields by matching literal, standard section headers — a header like this is not recognized as an Experience-type section by pattern-matching parsers, which creates real risk that the entire PixelWorks role (and its dates, for tenure/gap calculations) is dropped from the parsed candidate profile entirely. (Mechanically, `scripts/review_cv.py`'s `score_ats_compatibility` doesn't detect this on its own — it only checks for the *presence* of a standard "Experience" header somewhere in the document, which "MY JOURNEY" doesn't remove since the first job already anchors one. This is exactly the kind of rogue-header risk that needs the manual read, not just the mechanical pass.)
...
Every experience bullet describes an assigned duty rather than an outcome, and the one differentiating asset in the document — a personal project — is described in language too vague to convey any actual skill. Running the **So-What Test** (`references/bullet-audits.md`) against each bullet: all nine fail it — none survive the "okay... and?" gut check. No buzzword filler was found (`scripts/helpers.py`'s new `find_buzzwords` pass returned empty) — a small plus, since it means the fix here is purely about specificity and outcomes, not also stripping empty adjectives.

**Working well:** The role progression (Junior Developer → Software Developer) is a plausible, relevant story for an early-career software engineer, and the stack implied (backend systems, databases, APIs) is coherent for that target; no buzzword-filler language detected anywhere in the document.
...
**Evidence:** "Built an app for tracking inventory. Used some technologies to make it work." — this sentence could describe almost any project of any complexity; a technical screener gets zero signal from it. So-What check: "and?" — no answer follows.

#### Impact — 10/100
Zero of the nine bullets in this document contain a number, percentage, dollar figure, or concrete scale indicator of any kind — the **Numbers Audit** (`references/bullet-audits.md`) comes back empty on every axis (people, money, time, percentage, scale, rank).
...
**Evidence:** "Worked on improving API performance significantly" — "significantly" reads as an attempt at impact language but contains no actual number, which is a common trap: it sounds quantified without being checkable. This also fails the **Credibility Audit**, and live intake confirmed it directly: no real number was ever tracked for this work, so `cv-optimizer` cannot quantify this bullet — it stays a bracketed placeholder rather than getting a number, real or estimated.
...
**Evidence:** "I helped with building a new reporting dashboard for the team" — the only bullet in the document written in explicit first person; should read "Helped build..." or, if scope was larger than "helped," a stronger accurate verb. This bullet also fails the Credibility Audit as written: "helped with" doesn't establish what Maria's actual contribution was versus the team's — a gap the intake questions in `references/intake-questions.md` (specifically "what did you actually do day to day?") would resolve if a live candidate were available.
...
**Evidence:** "Used some technologies to make it work" is the single vaguest line in the document — it communicates literally nothing to the reader. Also the clearest So-What Test failure in the whole CV.
...
| Django | No | **Confirmed via live intake** — used for the Inventory Tracker App and backend-systems work; should be added |
| MySQL | No | **Confirmed via live intake** — the specific SQL dialect used; "SQL" alone undersells this |
| AWS | No | **Confirmed via live intake** — basic usage; should be added, honestly qualified |
| Testing framework / CI-CD | No | "Did testing for new features" implies some testing work with no named tool — not covered by this round of intake, still open |
...
Every row below fails the So-What Test as written; the "Audit" column notes which additional check (`references/bullet-audits.md`) applies beyond the baseline weak-phrase flag.

| Original | Issue | Audit | Suggested Rewrite |
|---|---|---|---|
| "Responsible for maintaining backend systems supporting the mobile app" | Duty-listing, no scope or outcome | Numbers | "Maintained [specific system(s)] supporting the mobile app, serving [X] requests/day" |
| "Helped with database migrations to a new schema" | Downplays contribution, no scale | Numbers + Credibility (what was the actual contribution vs. the team's?) | "Migrated [table/database] to a new schema, covering [X] records with [zero downtime / no data loss]" |
| "Worked on improving API performance significantly" | Vague intensifier standing in for a real number | Numbers + Credibility (is "significantly" defensible with a real figure?) | "Improved [specific endpoint]'s response time by [X]%" |
| "Participate in daily code reviews" | Present tense breaks convention used by surrounding bullets; low-signal as written | So-What | Consider folding into a broader collaboration bullet with real scope, or cutting if it doesn't add differentiating signal |
| "I helped with building a new reporting dashboard for the team" | First-person slip; downplays contribution; no scope | Numbers + Credibility | "Built [specific piece] of a new reporting dashboard used by [X] team members" |
| "Was in charge of fixing bugs in the production system" | Weak phrase; no scale | Numbers | "Resolved [N] production bugs in [system], reducing [incident rate / customer reports] by [X]%" |
| "Assisted with onboarding new team members" | Downplays contribution, no scale | Numbers | "Onboarded [N] new team members by [creating documentation / leading training sessions / pairing]" |
| "Did testing for new features" | Weak verb, no method or scale named | Numbers | "Tested [N] new features using [specific method/tool], catching [N] defects before release" |
| "Built an app for tracking inventory. Used some technologies to make it work." | Extremely vague; no tech stack, scope, or outcome | So-What (fails hardest of all nine) | "Built an inventory tracking app using [tech stack], supporting [scope — e.g., X SKUs, X users] [and achieving Y outcome, if any]" |

No bullet triggered the Buzzword Audit — Maria's flaw pattern is duty-listing and vagueness, not buzzword filler, so that audit contributes a clean pass rather than new findings here.
...
0 of 9 bullets (0%) contain any number, percentage, or scale indicator — matching the Numbers Audit result above. This is the primary driver of the Impact score and should be the first thing addressed — even directionally honest estimates ("roughly 10 teammates," "cut load time from ~3s to ~1s") would move this from Poor into at least Adequate territory.
...
3. Clean spelling and a genuinely simple, unadorned document with no ATS-hostile graphics, tables, or columns to strip out — and, per the Buzzword Audit, no filler-adjective padding either.

#### Biggest Weaknesses
1. Zero quantification anywhere in the document (0 of 9 bullets) — Numbers Audit fails across the board.
2. A non-standard section header ("MY JOURNEY") that puts an entire job at real risk of being dropped from ATS parsing.
3. Every single bullet opens with a flagged weak verb or duty-listing phrase, and every one fails the So-What Test.
...
4. Rewrite the Projects entry with an actual tech stack, scope, and outcome — currently the vaguest line in the document and the clearest So-What Test failure.
...
### Reliability note (this re-run vs. the original Day-5 run)

Same source document, same rubric and weights → identical category scores and overall score (46/100), which is the expected and desired outcome: the enrichment added evidence depth without changing the scoring logic itself — scores describe the CV as submitted, and live intake answers (gathered afterward, to inform the rewrite) correctly don't retroactively move them.

What live intake actually changed: the Credibility Audit's flag on "Worked on improving API performance significantly" went from a *suspected* gap to a *confirmed* one — no number was ever tracked, full stop. The Technical Skills / Keyword Optimization sections went from "likely underreporting, confirm and add if accurate" to "confirmed: Django and AWS were used, just never listed." That's the actual value of running the intake step for real instead of assuming it away, and it's a materially different (and more useful) report than the version that skipped this step entirely.
