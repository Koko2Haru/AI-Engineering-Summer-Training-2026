...
Each problem must have ≥ 2 visible tests and ≥ 3 hidden tests. **Every test must have
exactly one correct output** — no inputs where multiple different answers are valid
(e.g. a Two Sum input containing more than one pair that reaches the target) — and
every `expected` value must be verified correct by actually solving the test. Problem
topics are the builder's choice (arrays, strings, math — classic CP staples), but
statements must be original wording.
...
- A **Start** button begins the session: the chosen number of problems is drawn from the built-in set **without repetition** — a problem may appear at most once per session (choosing 6 uses all 6 problems). Order: mixed difficulties, hardest last. The session timer starts.
...
- A monospace `<textarea>` pre-filled with the problem's signature as a stub:
  ...
  ```
- **Run** button — executes against **visible tests only**; results appear per test: pass/fail, and on fail: input, expected, actual (or the error message). **Exception:** if the code fails to parse or does not define `solve`, show a single global error message instead of repeating the same error per test.
...
- [ ] A session of 6 contains 6 distinct problems — no duplicates in any session
- [ ] Code with a syntax/parse error on **Run** shows one global error, not the same error repeated per test
